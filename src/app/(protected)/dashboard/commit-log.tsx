      ...
      <ul className='space-y-6'>
        {commits?.map((commit, commitIndex) => {
          return <li key={commit.id} className='relative flex gap-x-4'>
            ...
            <>
              {/* no avatar, so use a temporary profile as fallback */}
              <img src={commit.commitAuthorAvatar || "profile.webp"} alt='commit avatar' className='relative mt-4 size-8 flex-none rounded-full bg-gray-50' />
